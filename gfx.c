...
void GFX_init() 
{
    ...
    disp_buffer = (uint32_t*)sceGeEdramGetAddr() + (272 * 512 * sizeof(uint32_t));
...
void GFX_draw_rect(unsigned int x, unsigned int y, unsigned int w, unsigned int h, uint32_t colour)
{
    if(x > 480)
    {
        x = 480;
    ...
    for(int y1 = 0; y1 < h; y1++) 
    {
        ...
        {
            draw_buffer[x1 + off + y1 * 512] = colour;
        }
    }
}